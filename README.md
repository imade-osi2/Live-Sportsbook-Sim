...
## Key Sportsbook Questions the Dashboard Answers

### Where is the best number right now?
Use `agg_best_price_by_outcome` to compare the best and worst available prices across books for each side and market.

### Are we finding prices that beat the market later?
Use `fact_bet_evaluation` and `agg_clv_kpis` to track beat-close count, beat-close percentage, and how often suggestions hold up against later observed prices.

### Which games deserve attention first?
Use `agg_research_board` and `agg_suggested_bets_board` to rank games by expected value, edge, and confidence.

### How does the market differ across books?
Use the Live Market Board and best-price views to identify price gaps and line-shopping opportunities.
...
The repo intentionally keeps both paths:
...
This README reflects the **updated dashboard plan** and the intended end-state for the capstone. It is meant to guide the next implementation steps while clearly communicating the problem, architecture, reproducibility approach, and final analytics experience.
